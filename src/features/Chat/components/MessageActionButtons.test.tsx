...
describe("MessageActionButtons", () => {
  ...
  const renderComponent = (props?: Partial<ComponentProps>) => {
    ...
    return render(
      <MessageActionButtons
        {...defaultProps}
        {...(props as Partial<ComponentProps>)}
      />
    );
  };
  ...
  it("shows the check icon briefly after copying and then hides it", async () => {
    ...
    const buttons = screen.getAllByRole("button");
    const initialInner = buttons[0].innerHTML;
    ...
    expect(buttons[0].innerHTML).not.toBe(initialInner);

    // wait slightly longer than the component timeout (1000ms) for it to reset
    await new Promise((res) => setTimeout(res, 1100));

    expect(buttons[0].innerHTML).toBe(initialInner);
  });
